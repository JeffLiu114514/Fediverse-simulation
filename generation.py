...
<<<<<<< HEAD
        if DEBUG: print(f"Post {post_counter} made by user {user.index} with quality {quality}")
        
        return post
    
    def post_reactions(self, post, warmup_sample_size=0.6):
        poster = post.user_index
        poster = self.users[poster]
        quality = post.quality
        variance, beta1, beta2, beta3, engagement_threshold = self.post_hyperparams
        full_visibility = set(poster.followers).union(poster.second_visibility).union(poster.third_visibility) # all people who can see the post

        #generate existing reactions to the post by warmup, collect subjective user scores on test sample
        warmup_sample = set(random.sample(poster.followers, int(warmup_sample_size * len(poster.followers))))
=======
...
<<<<<<< HEAD
        if DEBUG: print(f"Post {post.index} has {len(warmup_sample)} warmup sample and {len(test_sample)} test sample")
        
        for user in list(warmup_sample):
            observed_quality = np.random.normal(quality, variance)
            post.true_observed_quality[user] = observed_quality
            if observed_quality >= engagement_threshold:
=======
...
<<<<<<< HEAD
                ...
                if other_user in user_obj.first_visibility:
                    true_score += beta1 * post.true_observed_quality[other_user]
                elif other_user in user_obj.second_visibility:
                    true_score += beta2 * post.true_observed_quality[other_user]
                elif other_user in user_obj.third_visibility:
                    true_score += beta3 * post.true_observed_quality[other_user]
=======
                if other_user in user_visibility:
                    true_score += post.true_observed_quality[other_user]
>>>>>>> 984262a486ae26b52d184a49771aec76c744420e
